...
async function productsHandler(event) {
  ...
  const nextToken = start + items.length < products.length ? String(start + items.length) : void 0;
  return jsonResponse(200, {
    items,
    limit,
    nextToken,
...
async function createOrderHandler(event) {
  ...
  try {
    ...
    const orderItems = body.items.map((item) => {
      ...
      if (!Number.isFinite(quantity) || quantity < 1 || quantity > 1e4) {
  ...
  for (const record of event.Records) {
    ...
    const templateData = {
      ...
      itemsHtml: items.map((item) => {
        const displayName = item.productName?.trim() ? item.productName : item.productId;
        const unitPrice = typeof item.unitPrice === "number" ? (item.unitPrice / 100).toFixed(2) : item.unitPrice;
        return `<tr>
    ...
    const key = `orders/${message.orderId}/${Date.now()}-${(0, import_node_crypto2.randomUUID)()}.json`;
    try {
      await s3.send(
...
async function orderLakeHandler(event) {
  ...
  for (const record of event.Records) {
    ...
    const payload = {
      ...
      status: message.status,
      total: message.total,
      items: message.items !== void 0 ? JSON.stringify(message.items) : void 0,
      userPk: message.userPk
    };
    try {
      await kinesis.send(
